(function() {
    tinymce.create('tinymce.plugins.smallImageUploadPlugin', {
        init : function(ed, url) {
            ...
            // css.href = '/lib/tinymce/widgetPlugins/imageupload/css/style.css';
            head.appendChild(css);

            // Register commands
            ed.addCommand('mceSmallImageUpload', function() {
                            ...
                            if(response.code=="success"){
                                ...
                                if(jumpIsTrue) {
                                    ...
                                    for(var i = 0, len = response.model.length; i<len; i++) {
                                        var tpl = '<p><a href="'+jumpAddress+'"><img src="%s" class="insertSmallimg" data-role="jumpImg" /></a></p>';
                                        imgTpl.push(tpl.replace('%s', response.model[i]));
                                    }
                                }  else if(goodJumpIsTrue) {
                                    for (var i = 0, len = response.model.length; i < len; i++) {
                                        var tpl = '<p><a href="http://m.zhefengle.cn/#/detail&'+goodJumpInput+'&" class="siteBox item-detailLink"><img src="%s" class="insertSmallimg" data-role="jumpImg"/></a></p>';
                                        imgTpl.push(tpl.replace('%s', response.model[i]));
                                    }
                                }else {
                                    for (var i = 0, len = response.model.length; i < len; i++) {
                                        var tpl = '<p><img src="%s" class="insertSmallimg" /></p>';
                                        imgTpl.push(tpl.replace('%s', response.model[i]));
                                    }
                                }
            ...
            // Register buttons
            ed.addButton('smallImageupload', {
                title : '添加小图片',
                cmd : 'mceSmallImageUpload',
                // image : url + '/img/icon-small.png',
                image : './lib/tinymce/widgetPlugins/imageupload/img/icon-small.png'
    ...
    tinymce.PluginManager.add('smallImageupload', tinymce.plugins.smallImageUploadPlugin);
})();
